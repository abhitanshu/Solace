Feature: Calculate Fees
  ...
  Scenario:
    ...
    * def feeCalculationAmountClos = createAnnex2ByContractPayload.guarantor_participation_percentage * 0.01 * createAvailmentPayloadClos.outstanding_liability_amount
    ...
    * def numberOfDaysAmnd = testUtil.calculateDateDifference(createAvailmentPayloadAmnd.day,createAvailmentPayloadClos.day)
    * def numberOfDaysClos = testUtil.calculateDateDifference(createAvailmentPayloadClos.day,currentDate)
    ...
    * def expectedFeesClos = Number((numberOfDaysClos * feeCalculationAmountClos * calculatedBpsValue / 10000 / daysInYear()).toFixed(2))
    * configure headers = { "X-API-Key": #(importKey) }
    ...
    And match $[2].day_count == numberOfDaysClos
    ...
    * def actualFeesClos = formatNumber(response[2].amount)
    And match actualFees == expectedFees
    And match actualFeesAmnd == expectedFeesAmnd
    And match actualFeesClos == expectedFeesClos
