@SDA @TradeFinance
Feature: Trade Finance Amendment use case with Clos event for payment "At Maturity" when Checker approves the request created by Maker
  ...
  Scenario Outline: For Amendment use case Checker approves the request for <paymentFrequency> Payment schedule and currency <currency>
...
#Create an availment3 Clos event
    * availmentData.payload = createAvailmentPayloadClos
    * call read('classpath:features/common/CreateAvailment.feature')
...
#Calculate Remuneration fees
    * call read('classpath:features/common/CalculateFeesAmndClos.feature')
